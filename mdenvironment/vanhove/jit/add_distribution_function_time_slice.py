...
@njit(['Tuple((f4[:,:,:],f4[:,:,:,:]))(f4[:,:,:],f4[:,:,:,:],f4[:,:,:],i8[:,:],i8[:,:],i8[:],i8[:],f4[:,:,:],f4[:],UniTuple(f8,2),i8)'], parallel=True, fastmath=True, nogil=True, cache=True)
def _jit_append_Grts_general_mic(G_self, G_distinct, xyz, g1, g2, g1_lens, g2_lens, cuvec, cuvol, r_range, nbins):
    for i in prange(g1.shape[0]):
        for j in range(g2.shape[0]):
            rt_self, rt_distinct = _compute_rt_general_mic(xyz, g1[i][:g1_lens[i]], g2[j][:g2_lens[j]], cuvec)
...
def _jit_append_Grts_ortho_mic(G_self, G_distinct, xyz, g1, g2, g1_lens, g2_lens, cuvec, cuvol, r_range, nbins):
    for i in prange(g1.shape[0]):
        for j in range(g2.shape[0]):
            rt_self, rt_distinct = _compute_rt_ortho_mic(xyz, g1[i][:g1_lens[i]], g2[j][:g2_lens[j]], cuvec)
            G_distinct[i, j] += _compute_G_distinct(rt_distinct, cuvol, r_range, nbins)
        G_self[i] += _compute_G_self(rt_self, cuvol, r_range, nbins)
    return G_self, G_distinct


@njit(['f4[:,:,:](f4[:,:,:],f4[:,:,:],i8[:,:],i8[:],f4[:,:,:],f4[:],UniTuple(f8,2),i8)'], parallel=True, fastmath=True, nogil=True, cache=True)
